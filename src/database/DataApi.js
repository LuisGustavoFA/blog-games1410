...
export const getData = async (type) => {
  try {
    ...
    const data = type === 'news' ? [...dataJSON.news] : 
                 ...
                 type === 'reviews' ? [...dataJSON.reviews] : [...dataJSON.news, ...dataJSON.lists, ...dataJSON.reviews];
...
export const findArticle = async (title) => {
  try{
    ...
    const article = data.find((item) => item["title"].normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/,/g, "").replace(/-/g, " ").toLowerCase().includes(title));
    return article;
  } catch (error) {
